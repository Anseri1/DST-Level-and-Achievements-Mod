...
function allachivcoin:critcoin(inst)
    if self.coinamount >= self.critachivcost then
		if allachiv_coindata["crit"]*self.crit*100 >= 50 then
			return
		end
...
function allachivcoin:critfn(inst)
    inst:ListenForEvent("onhitother", function(inst, data)
        ...
        if target and math.random(1,100) <= chance and not target:HasTag("wall") and self.crit > 0 and self.attackcheck ~= true and data.target.components.health and data.target.components.health.currenthealth > 0 and data.target.components.combat then
...
function allachivcoin:icebodyfn(inst)
    ...
    --WX78 min temp is set back to -20 after losing overcharge
    if inst.prefab == "wx78" then
        inst:DoPeriodicTask(5, function()
            if self.icebody == true and inst.components.temperature.mintemp < 10 then
                inst.components.temperature.mintemp = 10
            end
        end)
    end
